...
function hasActiveStoreDiscount(game) {
  ...
  if (Number.isFinite(krw) && krw > 0) return original > krw;
  const usd = Number(game?.usd_price);
  if (Number.isFinite(usd) && usd >= 0) return original > usd;
  return false;
}

export function effectiveAffiliateDiscount(game, now = new Date()) {
  if (hasActiveStoreDiscount(game)) {
