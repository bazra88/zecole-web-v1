...
export async function inspectMetaStoreUrl(rawUrl) {
  ...
  if (!response.ok) throw new Error(`Meta 스토어 페이지를 읽지 못했습니다. (${response.status})`);
  ...
  const name = app?.name || relay?.display_name;
  const image = first(app?.image || app?.thumbnailUrl);
  const imageUrl = typeof image === "string" ? image : image?.url || relay?.hero_image?.uri || relay?.cover_square_image?.uri;
