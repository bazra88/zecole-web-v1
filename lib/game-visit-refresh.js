...
import { relayApp, parseKrw, extractBaseInfo, extractOfferPricing, extractLongDescription, translateLongDescription } from './meta-collect.mjs';
...
export async function refreshGameOnVisit(gameId) {
  ...
  let observation = {}, mediaChanged = false, saved = null;
  try {
    ...
    if (!claim.refresh_daily) observation.outcome = 'ok';
      ...
      observation = {...observation, price:price.price,currency,original_price:offer.originalPrice,
        offer_ends_at:offer.offerEndsAt,show_timer:offer.showTimer,outcome:'ok'};
    } else {
  ...
  return {...result,changed:Boolean(result.changed || mediaChanged),failed:observation.outcome !== 'ok'};
}
