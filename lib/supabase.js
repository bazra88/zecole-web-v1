import { displayedDiscountPercent } from "./game-format.js";
...
export async function getGames({
  ...
  if (sortDiscount) {
    // Rank the full filtered catalog before pagination, using the card's price rules.
    // Fetch only ranking fields in cached batches; load full cards for this page.
    const candidates = [];
    let total = null;
    do {
      const batch = await restSelect("games", {
        ...params,
        select: "id,name,pricing_type,affiliate_url,current_price,currency,krw_price,usd_price,meta_store_original_price,base_affiliate_discount_percent,affiliate_discount_percent,promo_affiliate_discount_percent,promo_starts_at,promo_ends_at",
        order: "id.asc", limit: 1000, offset: candidates.length,
      }, { count: true, revalidate });
      total = batch.count;
      if (!batch.data.length) break;
      candidates.push(...batch.data);
    } while (total == null || candidates.length < total);
    const now = new Date();
    candidates.sort((a, b) => displayedDiscountPercent(b, now) - displayedDiscountPercent(a, now)
      || a.name.localeCompare(b.name, "ko") || a.id.localeCompare(b.id));
    const ids = candidates.slice(offset, offset + limit).map(game => game.id);
    if (!ids.length) return { data: [], count: candidates.length };
    const page = await restSelect("games", {
      ...params, id: `in.(${ids.join(",")})`, offset: 0, limit: ids.length,
    }, { revalidate });
    const positions = new Map(ids.map((id, index) => [id, index]));
    page.data.sort((a, b) => positions.get(a.id) - positions.get(b.id));
    return { data: page.data, count: candidates.length };
  }
...
export async function getGameMedia(gameId) {
  const { data } = await restSelect(
    "game_media",
    {
      select: "*",
      game_id: `eq.${gameId}`,
      active: "eq.true",
      order: "media_type.asc,sort_order.asc",
    },
    { revalidate: 300 }
  );
  return data || [];
...
export async function getGameReviews(gameId, page = 1, {revalidate = 300} = {}) {
  const { data, count } = await restSelect('game_reviews', {
    select:'id,reviewer_label,rating,title_original,body_original,title_ko,body_ko,helpful_count,reviewed_at',
    game_id:`eq.${gameId}`,active:'eq.true',
    order:'helpful_count.desc.nullslast,reviewed_at.desc.nullslast,id.asc',
