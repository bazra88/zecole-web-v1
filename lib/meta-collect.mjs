...
// 메타 CDN 썸네일 URL의 서명 토큰(_nc_ohc/oh/oe 등)은 요청할 때마다 바뀌지만, 경로(파일명)에는
// 실제 미디어 자산 ID가 박혀있어서 파일이 진짜 교체되지 않는 한 그대로다 — 쿼리스트링을 뺀
// "경로만" 비교하면 재다운로드 없이도 썸네일이 실제로 바뀌었는지 판단할 수 있다.
...
export function parseKrw(html, metaId, relay) {
  ...
  const rawAmount = relay?.current_offer?.price?.offset_amount;
...
// 무료 구글 번역(브라우저 확장이 쓰는 것과 같은 비공식 엔드포인트, API 키 불필요) — 비공식
// 엔드포인트라 이따금 타임아웃/일시 오류가 나는데, 재시도가 없어서 한 번만 실패해도 그
// 문단(전체 설명이면 그 게임 전체)이 원문 그대로 저장되는 채로 영영 남아있었다(2026-09-23,
// 실제로 description_long_ko의 46%가 이렇게 번역 안 된 채로 남아있던 걸 발견하고 추가).
const TRANSLATE_RETRY_DELAYS_MS = [500, 1500];
export async function translateText(text, target = "ko") {
  ...
  for (const chunk of chunks) {
    ...
    let ok = false;
    for (let attempt = 0; attempt <= TRANSLATE_RETRY_DELAYS_MS.length && !ok; attempt++) {
      try {
        ...
        if (!response.ok) throw new Error(`번역 요청 실패 (${response.status})`);
        const data = await response.json();
        translated.push((data?.[0] || []).map((part) => part?.[0] || "").join(""));
        ok = true;
      } catch {
        if (attempt < TRANSLATE_RETRY_DELAYS_MS.length) await sleep(TRANSLATE_RETRY_DELAYS_MS[attempt]);
      }
    }
    if (!ok) return null;
...
// zecole.store에서 실제로 필요한 최대 이미지 크기 — 상세페이지 히어로 이미지(.detail-image,
// 16:9)와 스크린샷 갤러리가 가장 큰 사용처인데, 화면에 꽉 채워도 720p(1280px 장변)면
// 육안으로 흐려 보이지 않는 수준이라 판단해서 용량을 더 줄였다(2026-09-09에 1600px/q82로
// 한 번 낮췄었는데, 스크린샷까지 전부 자체저장하기로 하면서 용량 부담이 커져 2026-09-23에
// 1280px/q75로 한 번 더 낮춤 — 신규 업로드부터 적용되고 기존 파일은 resizeStoredImageIfNeeded가
// 자연스럽게 재방문 시 줄인다).
export const MAX_IMAGE_DIMENSION = 1280;
export const IMAGE_WEBP_QUALITY = 75;

async function resizeToWebpIfNeeded(buffer, { maxDimension = MAX_IMAGE_DIMENSION, quality = IMAGE_WEBP_QUALITY } = {}) {
  const image = sharp(buffer);
  const metadata = await image.metadata();
  const needsResize = (metadata.width || 0) > maxDimension || (metadata.height || 0) > maxDimension;
  const needsFormatChange = metadata.format !== "webp";
  if (!needsResize && !needsFormatChange) return { buffer, width: metadata.width, height: metadata.height, changed: false };
  const resized = await image
    .resize({ width: maxDimension, height: maxDimension, fit: "inside", withoutEnlargement: true })
    .webp({ quality })
    .toBuffer({ resolveWithObject: true });
  return { buffer: resized.data, width: resized.info.width, height: resized.info.height, changed: true };
}
...
// 보고 발견). 업로드 전에 필요 이상으로 큰 이미지는 축소한다(MAX_IMAGE_DIMENSION 참고).
// 실패해도 예외를 던지지 않고 null을 반환한다 — 호출자는 최소한 source_image_url은
// 이미 저장했을 것이므로 이번엔 그냥 넘어가도 된다.
export async function uploadImageToStorage({ imageUrl, path, supabaseUrl, supabaseSecretKey, bucket = "game-images", maxDimension, quality }) {
  ...
  try {
    ...
    let contentType = imageResponse.headers.get("content-type") || "image/webp";
    try {
      const resized = await resizeToWebpIfNeeded(buffer, { maxDimension, quality });
      if (resized.changed) { buffer = resized.buffer; contentType = "image/webp"; }
    } catch { /* 리사이즈 실패(손상된 이미지 등)해도 원본 그대로 업로드는 시도한다 */ }
...
// 메타 원본이 그때는 안 커서(또는 리사이즈 로직이 생기기 전이라) 그대로 저장된 기존 썸네일을
// 다시 훑어서, 지금 기준(MAX_IMAGE_DIMENSION)보다 크면 우리 Storage 사본만 내려받아 줄여서
// 같은 경로에 덮어쓴다 — 메타에 다시 요청하지 않으므로 차단 위험이 전혀 없다.
export async function resizeStoredImageIfNeeded({ path, supabaseUrl, supabaseSecretKey, bucket = "game-images", maxDimension, quality }) {
  if (!path || !supabaseUrl || !supabaseSecretKey) return { resized: false };
  try {
    const objectUrl = `${supabaseUrl.replace(/\/+$/, "")}/storage/v1/object/${bucket}/${path}`;
    const response = await fetch(objectUrl, { signal: AbortSignal.timeout(20_000) });
    if (!response.ok) return { resized: false };
    const buffer = Buffer.from(await response.arrayBuffer());
    const resized = await resizeToWebpIfNeeded(buffer, { maxDimension, quality });
    if (!resized.changed) return { resized: false, width: resized.width, height: resized.height };
    const uploadResponse = await fetch(objectUrl, {
      method: "POST",
      headers: { apikey: supabaseSecretKey, Authorization: `Bearer ${supabaseSecretKey}`, "Content-Type": "image/webp", "x-upsert": "true" },
      body: resized.buffer,
      signal: AbortSignal.timeout(20_000),
    });
    return { resized: uploadResponse.ok, width: resized.width, height: resized.height };
  } catch {
...
// extractMedia()가 뽑아준 스크린샷/트레일러 목록을 받아서, 스크린샷은 썸네일과 똑같이
// 자체 Storage에 다운로드해 영구 저장한다(장당 수십 KB라 부담이 적고, 한 번 저장하면
// 메타 CDN 서명이 만료돼도 다시는 깨지지 않는다). 트레일러(mp4, 게임당 최대 1개, 평균
// ~12MB)는 전부 다운로드하면 스토리지가 수십 GB 단위로 불어나서 저장하지 않고, 포스터
// 이미지만 자체저장한 뒤 원본 CDN 링크는 그대로 둔다 — 실제 재생은 사용자가 재생 버튼을
// 누른 순간 app/api/media/trailer/route.js가 그 시점 링크를 즉석에서 다시 조회해서
// 처리한다(2026-09-23, 스토리지 용량 대 링크 만료 문제를 절충).
export async function persistMedia(media, { metaId, supabaseUrl, supabaseSecretKey, bucket = "game-images" }) {
  // 게임 상세페이지 첫 방문자가 이 전체를 기다려야 하는 경우(lib/game-media-backfill.js)가
  // 있어서, 장당 순차 다운로드 대신 전부 병렬로 처리해 체감 대기시간을 줄인다.
  const trailer = media.find((item) => item.media_type === "trailer");
  const screenshots = media.filter((item) => item.media_type === "screenshot");

  const [trailerResult, screenshotResults] = await Promise.all([
    trailer
      ? (async () => {
          const posterPath = `media/${metaId}/trailer-poster.webp`;
          const stored = trailer.thumbnail_url
            ? await uploadImageToStorage({ imageUrl: trailer.thumbnail_url, path: posterPath, supabaseUrl, supabaseSecretKey, bucket })
            : null;
          return { ...trailer, thumbnail_url: stored ? publicStorageUrl(supabaseUrl, bucket, stored) : trailer.thumbnail_url };
        })()
      : Promise.resolve(null),
    Promise.all(screenshots.map(async (item, index) => {
      const path = `media/${metaId}/${index}.webp`;
      const stored = await uploadImageToStorage({ imageUrl: item.url, path, supabaseUrl, supabaseSecretKey, bucket });
      if (!stored) return item; // 다운로드 실패 시 원본 CDN 링크라도 남겨둔다(곧 만료되긴 하지만 아예 없는 것보단 낫다)
      const publicUrl = publicStorageUrl(supabaseUrl, bucket, stored);
      return { ...item, url: publicUrl, thumbnail_url: publicUrl };
    })),
  ]);

  return [...(trailerResult ? [trailerResult] : []), ...screenshotResults];
}

// current_offer.strikethrough_price/end_time/show_timer — 할인 배너가 떠 있을 때 메타가
// 원가/세일종료시각/타이머표시여부를 얹어서 주는 필드(2026-09-09, VPS에서 relay 실제
// 응답 구조 직접 확인). strikethrough_price는 price와 같은 {currency, offset_amount}
// 형태이거나, 세일이 없으면 null이다.
export function extractOfferPricing(relay) {
  const offer = relay?.current_offer;
  if (!offer) return { originalPrice: null, showTimer: false, offerEndsAt: null };
  const strikethrough = offer.strikethrough_price;
  const amount = Number(strikethrough?.offset_amount);
  const originalPrice = strikethrough && Number.isFinite(amount) ? amount / 100 : null;
  const endTime = Number(offer.end_time);
  return {
    originalPrice,
    showTimer: Boolean(offer.show_timer),
    offerEndsAt: Number.isFinite(endTime) && endTime > 0 ? new Date(endTime * 1000).toISOString() : null,
  };
}
...
export async function collectGameData({ html, metaId, existingGame, skipDescriptionTranslation, existingReviewIds, hasExistingMedia }) {
  ...
  // storeResolved가 false(예: 아직 coming_soon이라 KR 판매 여부 자체가 불확실한 경우)면
  // krw_store_available/region_restricted를 결정할 수 없다 — 이때 예전엔 existingGame이
  // 있으면 무조건 이 셋을 채워서 region_restricted에 null이 들어갔는데, 그 컬럼이 DB에서
  // NOT NULL이라 PATCH/INSERT가 23502로 거부됐다(2026-09-08, 아직 출시 안 된 신작들을
  // 로컬에서 갱신하다가 발견). storeResolved일 때만 채우고, 아니면 셋 다 빼서 기존 값을
  // 그대로 둔다(신규 게임 INSERT라면 region_restricted는 DB 기본값 false를 그대로 씀).
  if (storeResolved) {
