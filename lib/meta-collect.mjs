...
//
// ⚠️ 반드시 한국 IP에서 실행되는 코드에서만 import할 것 (메타는 요청 IP의 실제 국가로
// 통화/언어를 결정한다). Vercel(미국 IP)에서 이 모듈을 직접 fetch용으로 쓰면 안 되고,
// 반드시 한국 IP를 가진 서버(로컬 실행 / 서울 클라우드 서버)를 거쳐야 한다.
...
export function extractBaseInfo(html, relay, metaId) {
  ...
  const imageUrl = typeof image === "string" ? image : image?.url || relay?.hero_image?.uri || metaContent(html, "og:image") || null;
...
export async function collectGameData({ html, metaId, existingGame, skipDescriptionTranslation, existingReviewIds }) {
  ...
  let baseInfo = null;
  if (!existingGame) {
    baseInfo = extractBaseInfo(html, relay, metaId);
