...
async function save() {
  ...
  const summary={source:url,expected_currency:expected,started_at:new Date(started).toISOString(),finished_at:new Date().toISOString(),complete,reason,count:items.length,priced:items.filter(r=>r.current_price!=null).length,discounted:items.filter(r=>r.current_price!=null&&r.original_price>r.current_price).length,wrong_currency:items.filter(r=>r.currency&&r.currency!==expected).length,ambiguous_prices:items.filter(r=>r.price_ambiguous).length,json_responses:jsonResponses,offer_objects:offerObjects,offers_with_end_time:[...offers.values()].filter(o=>o.end_time!=null).length,network:Object.fromEntries(network),progress};
